...
def test_fish_audio_tts(text: str, voice: str = None, output_dir: str = None):
    """fish.audio TTSのテスト
    ...
    try:
        ...
        output_path = tts_engine.generate_speech(
            text,
            voice=voice,
            output_filename=f"test_fish_audio_{voice}.mp3"
        )
        ...
        logger.error(f"Error in test_fish_audio_tts: {e}")
        raise

if __name__ == "__main__":
    # テスト用のテキストを指定
    test_text = "これはfish.audio TTSのテストです。音声合成の品質を確認しています。"
    ...
    # テスト実行
    test_fish_audio_tts(test_text, voice=test_voice)
